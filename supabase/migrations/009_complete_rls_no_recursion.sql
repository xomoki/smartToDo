...
-- 2. このファイル (009_complete_rls_no_recursion.sql) を実行
...
-- 組織メンバーの閲覧: 
-- 1. 自分自身のレコードは常に閲覧可能
-- 2. 自分が所属する組織のメンバーも閲覧可能
--    重要: organization_membersを直接参照せず、
--    organizationsテーブルのポリシーを利用して判断する
--    organizationsテーブルのSELECTポリシーにより、
--    自分がメンバーである組織のみが返されるため、
--    その組織のメンバーも閲覧可能になる
CREATE POLICY "Users can view organization members" ON organization_members
    FOR SELECT USING (
        ...
        -- 自分がメンバーである組織のみが返される
        -- そのため、その組織のメンバーも閲覧可能
        -- この方法により、organization_membersを直接参照せずに済む
        EXISTS (
            SELECT 1 
...
-- 注意: organization_membersを参照するが、usersテーブルのポリシーなので、
-- organization_membersのポリシー評価には影響しない（再帰は発生しない）
